...
import math
class Point:
    """A point located at (x,y) in 2D space. Each Point object may be associated with a payload object."""
    ...
    def __str__(self):
        return 'P({:.6f}, {:.6f})'.format(self.x, self.y)
...
class Rect:
    ...
    def intersects(self, other):
        """Does Rect object another intersect this Rect?"""
...
class QuadTree:
    ...
    def true_coors(nrx, nry, cx, cy):
        #this is how we change the points from the rectangle to coordinates
        #this is NE
        if nrx > 180.0 and nry < 90.0:
            tcoorx = nrx - 180.0
            tcoory = 90.0 - nry
        #this is NW
        if nrx < 180.0 and nry < 90.0:
        ...
        cx = abs(cx)
        cy = abs(cy)
        #this calculates the distances between the coordinates
        dis = math.sqrt(pow((tcoorx - cx),2)+pow((tcoory - cy),2))
        return dis
    def switch(option):
        #This takes the output of their dicision and and outputs the option form closest to furthest
        if option == 1:
            return "Thanks for choosing the the first Option. "
        elif option == 2:
            return "Thanks for choosing the the second Option. "
        elif option == 3:
            return "Thanks for choosing the the third Option. "
        elif option == 4:
            return "Thanks for choosing the the forth Option. "
        elif option == 5:
            return "Thanks for choosing the the fifth Option. "
        else:
